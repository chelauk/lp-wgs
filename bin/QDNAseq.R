...
write.table(calls_adj, file = paste0(patient,"_",sample,"_cna_calls.txt"), quote = FALSE, sep = "\t")

# Write out the segments with mean logr for extra fun
write.table(segmented(cgh), file = paste0(patient,"_",sample,"_cna_segments.txt"), quote = FALSE, sep = "\t")

# Read in the data
lrrs = read.table(paste0(patient, "_",sample,"_bins.txt"), header = TRUE)
call = read.table(paste0(patient, "_",sample,"_cna_calls.txt"), header = TRUE)

# Play with levels
call = as.factor(call[,1])
call = factor(call, levels = c("-2", "-1", "0", "1", "2"))

segs = read.table(paste0(patient,"_",sample,"_cna_segments.txt"), header = TRUE)
...
write.table(segs, file = paste0(patient,"_",sample,"_cna_segments.txt"), quote = FALSE, sep = "\t")
