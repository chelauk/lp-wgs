...
get_chr_signal <- function(copy_numbers_normalized, chr, sample_col = 1) {
  cn_mat <- copynumber(copy_numbers_normalized)
  ...
  idx <- bins_df$chromosome == chr & bins_df$use %in% TRUE
  
  y <- cn_mat[idx, sample_col] * 2
  bins_chr <- bins_df[idx, ]
...
run_bayes_chr <- function(copy_numbers_normalized, chr) {
  chr_data <- get_chr_signal(copy_numbers_normalized, chr)
  
  res <- run.BayesCNA(chr_data$y)
  
  breakpoint <- rep(FALSE, length(chr_data$y))
  breakpoint[res$partition] <- TRUE
  
  data.frame(
    chromosome = chr,
    start = chr_data$bins$start,
    end   = chr_data$bins$end,
    raw   = chr_data$y,
    profile = res$profile,
