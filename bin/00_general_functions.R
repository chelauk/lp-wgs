...
explode_ranges <- function(df, step_size) {
  # Create a list to hold each exploded range
  ...
  for (i in 1:nrow(df)) {
    ...
    # Generate sequence of starts and ends
    ...
    # Store the expanded rows in the list
  ...
  # Combine all the exploded ranges into one dataframe
  expanded_df <- do.call(rbind, exploded_list)
